...
internal class Application
{
    ...
    public void InitMainMenu()
    {
        ...
        this.mainMenu.AddItem("Get all orders by client id", this.GetOrdersByClientId, sfo);                    // not implemented | work
        this.mainMenu.AddItem("Find all orders by a product id", this.GetOrdersByProductId, sfo);               // not implemented | work
        this.mainMenu.AddItem("Show order details by order id", this.ShowOrderDetails, orders);
        this.mainMenu.AddItem("Delete an order", this.DeleteOrder, orders);                                     // not implemented | work
        ...
        this.mainMenu.AddItem("Edit product", this.EditProduct, products);                                     // not implemented  | work
        this.mainMenu.AddItem("Delete product", this.DeleteProduct, products);                                 // not implemented  | work

        var categories = this.mainMenu.AddItem("Categories");
        this.mainMenu.AddItem("New category", this.NewCategory, categories);
        this.mainMenu.AddItem("Show all categories", this.ShowAllCategories, categories);                       // not implemented | work
        this.mainMenu.AddItem("Edit category", this.EditCategory, categories);                                  // not implemented | work
        this.mainMenu.AddItem("Delete category", this.DeleteCategory, categories);                              // not implemented | work
        ...
        this.mainMenu.AddItem("Find client by last name or id", this.FindClientByLastNameOrId, fc);             // not implemented | work
        this.mainMenu.AddItem("Find client by email", this.FindClientByEmail, fc);                              // not implemented | work
        this.mainMenu.AddItem("Find client by phone number", this.FindClientByPhone, fc);                       // not implemented | work 
        this.mainMenu.AddItem("Edit client info", this.EditClientInfo, clients);                                // not implemented | work
        this.mainMenu.AddItem("Delete client", this.DeleteClient, clients);                                     // not implemented | work
    ...
    private void GetOrdersByClientId()
    {
        ...
        {
            Console.WriteLine(
                $"Order id: [{order.Id,-4}] By client: [{order.Client.LastName,-12}{order.Client.FirstName,-12}] {order.IssueDateTime.Date}"
                );
        }
        ...
        foreach (Order order in result)
        {
            ...
            {
                Console.WriteLine(
                    $" - Product: [{item.Product.Name,-60}] quantity: [{item.Quantity}] price: [{item.Price}]"
                    );
            }
        }
    }
    ...
    private void GetAllProductsByCategory()
    {
        ...
        {
            Console.WriteLine(
                $"product id: [ {product.Id,-4} ] name: [ {product.Name,-40} ] price: [ {product.Price} ]"
                );
        }
    }
    ...
    private void FindClientByPhone()
    {
        ...
        Console.WriteLine($"Id            : {client.Id}\n" +
                          ...
                          $"Phone         : {client.Phone}\n" +
                          $"Orders        : {
                              (client.Orders.Count < 1 ? "<NO ORDERS>" : string.Join(", ", client.Orders.Select(o => o.Id).ToList()))
                              }");
    }
